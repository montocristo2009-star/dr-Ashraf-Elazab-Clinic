      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16 animate-fade-in-up">
          ...
          <p className="text-lg text-slate-500 font-bold max-w-2xl mx-auto italic">
            "جولة بصرية حقيقية من داخل غرف العمليات والعيادة، توثق دقة العمل والتجهيزات الحديثة."
          </p>
        </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-8 py-3 rounded-full font-black text-xs transition-all ${
                ...
                : 'bg-white text-slate-400 border border-slate-100 hover:text-medical-green'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredImages.map((img, i) => (
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-medical-blue/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-8">
          ...
          <div className="fixed inset-0 z-[3000] bg-medical-blue/90 backdrop-blur-md flex items-center justify-center p-6" onClick={() => setSelectedImage(null)}>
            <div className="max-w-5xl w-full flex flex-col items-center animate-fade-in" onClick={e => e.stopPropagation()}>
              <img 
                ...
                className="max-h-[80vh] object-contain rounded-[40px] shadow-2xl border-4 border-white/10" 
              />
              <div className="mt-8 text-center">
                <h2 className="text-2xl font-black text-white">{selectedImage.title}</h2>
                <p className="text-medical-green font-bold mt-2">عيادة {DOCTOR_NAME}</p>
              </div>
            </div>
          </div>
        )}
